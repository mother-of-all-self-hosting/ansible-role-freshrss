...
#
# Tags look like `v<FreshRSS version>-<release>`:
#
# - if defaults/main.yml points at a FreshRSS version that has never been
#   released, the release counter restarts at 0 (`v1.29.1-0`)
# - otherwise the counter is incremented (`v1.29.1-1`), but only if something
...
# Anchored on `freshrss_version:` so that neither `freshrss_container_image_tag:`
# (whose value refers to `freshrss_version`) nor a commented-out line can be
# mistaken for it.
version="$(sed -nE 's|^freshrss_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the FreshRSS version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (e.g. `1.29.1`) while the tags do,
# and a value that gained one must not end up doubling it.
