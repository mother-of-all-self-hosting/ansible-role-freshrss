...
Set `mysql` to use a MySQL compatible database and `sqlite` to use SQLite, respectively. The SQLite database is stored in the directory specified with `freshrss_data_path`.
...
freshrss_database_mysql_socket_path_host: ""

# Specify the path to the Postgres Unix socket path on the host (bind-mount source)
freshrss_database_postgres_socket_path_host: ""
```
...
freshrss_database_mysql_socket_enabled: false

# Disable the connection to the Postgres server via a Unix socket
freshrss_database_postgres_socket_enabled: false
```
